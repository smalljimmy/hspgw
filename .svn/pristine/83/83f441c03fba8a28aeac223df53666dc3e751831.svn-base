			     ...
			     RADIUS_ESCAPE_STRING func)
{
	...
	*pa = '\0';
	if (!*p) {
		...
		da = dict_attrbyname(xlat_string);
		if (da != NULL) {
			retlen = valuepair2str(q,freespace,pairfind(request->config_items,da->attr),PW_TYPE_STRING, func);
                        if (retlen == 1 && *q == '_') // not in config_items -> try packet
                            retlen = valuepair2str(q,freespace,pairfind(request->packet->vps,da->attr),PW_TYPE_STRING, func);
		}
		if (retlen > 0) found = 1;
 ...
 */
int parse_repl(char *out, int outlen, const char *fmt,
...
int convert2ascii(char *out, int outlen, char *in)
{
	int c, freespace;
	const char *p;
	char *q;
	...
	while (*p != 0) {
		...
		/*
		 *	There's nothing after this character
		 */
		if (*p == '\0') {
			break;
		}
		...
		if (c > 0 && c < 128) {
			//standard ascii, don't need to be converted
			*q++ = *p++;
			continue;
		}
		...
		switch(c) {
			...
			default:
				DEBUG("WARNING: Unknown variable '%c'", *p);
				if (freespace > 2) {
					*q++ = '?';
					p++;
				}
