...
static int rlm_pms_detach(void *instance) {
	...
	rlm_pms_2_module_t *inst = (rlm_pms_2_module_t *) instance;
	//    PMS_SOCKET_POOL *cur;
...
static int pms_instantiate(CONF_SECTION *conf, void **instance) {
	...
	rlm_pms_2_module_t * inst;

	DEBUG2("Entering PMS 2 Instantiation ...");
	DEBUG2("============================");
	...
	if ((inst->config->sql_instance_name == NULL) || (strlen(
		...
		radlog(
				L_ERR,
				"%s : rlm_pms_2 instantiation : The 'sql-instance-name' variable must be set.",
				"SC_HSPGW_CONFIG_ERR");
		rlm_pms_detach(inst);
		return -1;
	}
	...
	if (!modinst) {
		radlog(
				L_ERR,
				"%s : rlm_pms_2 instantiation : Failed to find sql instance named %s",
	...
	if (strcmp(modinst->entry->name, "rlm_sql") != 0) {
		radlog(
				L_ERR,
				"%s : rlm_pms_2 instantiation : Module \"%s\" is not an instance of the rlm_sql module",
				"SC_HSPGW_CONFIG_ERR", inst->config->sql_instance_name);
		rlm_pms_detach(inst);
		return -1;
	}
...
static int pms_authenticate(void *instance, REQUEST *request) {
	...
	DEBUG2("Entering PMS 2 Authentication ...");
	DEBUG2("===============================");
	// initalize log data
	...
	DEBUG2("rlm_pms_2::pms_authenticate: Reading required attributes.");
...
*/
	...
	DEBUG2(	"rlm_pms_2::pms_authenticate: Found attribute SCS-SPP-StrippedUser: '%s'.", vp->vp_strvalue);

	strcpy(trckrdata->usr, vp->vp_strvalue);
	strcpy(logfiledata->username, vp->vp_strvalue);

	// Extract User-Password attribute
	DEBUG2("rlm_pms_2::pms_authenticate: Extract password from User-Password attribute.");
	...
	DEBUG2("rlm_pms_2::pms_authenticate: Found attribute User-Password: '%s'.",
				vp->vp_strvalue);
	...
	{
		DEBUG2("rlm_pms_2::pms_authenticate: Found attribute Cisco-SSG-Account-Info: '%s'.",
					vp->vp_strvalue);
		...
		else
		{
			...
			DEBUG2("rlm_pms_2::pms_authenticate: Found AutoLogin=1 and WebProxy=%d", webproxy);
		}
	}
	...
	if (autoLogin > 0){
		...
		if (!vp) {
			autoLogin = 0;
			DEBUG2("rlm_pms_2::pms_authenticate: Reset AutoLogin");
			radlog(L_INFO, "%s : req n/a : sid %s : zid %s : usr %s : %s",  "SC_HSPGW_AUTH_MALFORMED_REQ",
		...
		else
		{
			...
			DEBUG2("rlm_pms_2::pms_authenticate: Found attribute Framed-IP-Address: '%s'.",strIpAddress);
		}
		...
		{
			DEBUG2("rlm_pms_2::pms_authenticate: Found attribute Calling-Station-Id: '%s'.",
						vp->vp_strvalue);
	...
	DEBUG2("rlm_pms_2::pms_authenticate: Purge database tables ACCOUNT_TRACKER_RELOGIN and ACCOUNT_TRACKER.");
...
//	}
	...
	// Recalculate Session Timeout value
		...
		DEBUG2(
					"rlm_pms_2::pms_authenticate: This is a new authentication request. Alternative authentication required.");
	}
	...
	if (occupancyId == -1) { // db error
		...
		DEBUG2("rlm_pms_2::pms_authenticate: Session Deadline value: %d",
					trckrdata->deadline);
	...
	for (list = pmsdatalist; list != NULL; list = list->next) {
		...
		if (pmssocket == NULL) {
			DEBUG2(
						"rlm_pms_2::pms_authenticate: find new IP %s:%s (new PMS?), update the socket pool",
						pmsdata->ipaddr, pmsdata->port);
...
static int pms_accounting(void *instance, REQUEST *request) {
	...
	// Extract Acct-Session-Id attribute
	DEBUG2("rlm_pms_2::pms_accouting: Checking required attributes...");
	vp = pairfind(request->packet->vps, PW_ACCT_SESSION_ID);
	...
	DEBUG2("rlm_pms_2::pms_accounting: Tariff id is %d, type is %s.", pmsdata->tariffid, pmsdata->tariff_key);
	...
	//DEBUG2("rlm_pms_2::pms_accounting: account tracker query delivered %d, %s, %s, %s", ret, row[0], row[1], row[2]);
	...
	if (atoi(pmsdata_checkin->amount) > 0) {
		...
		DEBUG(
					"rlm_pms_2::pms_accounting: posting amount of CHF %s to PMS %s:%s...",
		...
		if (pmssocket == NULL) {
			DEBUG2(
						"rlm_pms_2::pms_accounting : find new IP %s (new PMS?), update the socket pool",
		...
		pms_fias_infoLog(msgtxt, "SC_HSPGW_ACCT_TRANSMITTED", inst,
				logfiledata, request);
		DEBUG(
					"rlm_pms_2::pms_accounting: charge amount is CHF 0.00. Treat it as billed.");
		trf = 1;
	}

	// Update the billed flag in the account tracker table
	if (trf) {
		DEBUG("rlm_pms_2::pms_accounting: setting billed-flag ...");
		...
		DEBUG("rlm_pms_2::pms_accounting: flag set.");
	}
...
//	radlog(L_INFO, "pms_fias_verify_password (%s : %s): pusrfld: '%s', tusr: '%s', tpw: '%s'", pmsdata->ipaddr, pmsdata->port, pmsdata->usrfld, trckr->usr, trckr->pwd);
	...
	// sepcial case: hogatex systems return a PL with CT if the room was not found:
	if (pms_fias_check_packet_type(message, PROTEL_FIAS_RECID_PL) == 0
			&& packet->cleartxt != NULL && strcmp(packet->cleartxt, "No Room") == 0 ) // todo: put cleartext to config (pms_2)
	{
		//radlog(L_ERR,"pms_fias : %s : Protel PMS server response: '%s'.", "SC_HSPGW_AUTH_INVALID_USER", packet->cleartxt);
		free(packet);
		return 1;
...
static int pms_fias_proc_PR_posting(void *instance, pms_tracker_data **tracker,
	...
	if (!strcmp(packet->answstatus, PROTEL_FIAS_AS_OK)) {
		 ...
		 DEBUG("pms_fias::pms_proc_PR_posting: PR Posting successful: Amount: %s, Room: %s, Guest-Id: %s", pmsdata->amount, trckr->usr, trckr->guestid);
 ...
 *  @int *deadline: the deadline (output)
...
static int pms_fias_accTrackerFind(void *instance, REQUEST *request, int *deadline) {
	...
	SQL_ROW row;
	VALUE_PAIR *vp;
	rlm_pms_2_module_t *inst = instance;
	char querystr[MAX_QUERY_LEN];
	...
	sqlsocket = inst->sql_inst->sql_get_socket(inst->sql_inst);
	if (sqlsocket == NULL) {
		//radlog(L_ERR, "rlm_pms_2 : %s : Connection to data base failed.", "SC_HSPGW_AUTH_SPPDB_CONN_ERR");
		return -1;
	}
	...
	if (!vp) {
		...
		//radlog(L_ERR, "rlm_pms_2 : Missing SCS-SPP-DecodedPassword attribute.");
		return -2;
	}
	...
	if (inst->sql_inst->sql_select_query(sqlsocket, inst->sql_inst, selectquery)) {
		//radlog(L_ERR, "rlm_pms_2 : %s : Database select from query failed", "SC_HSPGW_AUTH_ORA_ERROR");
		(inst->sql_inst->module->sql_finish_select_query)(sqlsocket,
				inst->sql_inst->config);
		inst->sql_inst->sql_release_socket(inst->sql_inst, sqlsocket);
		return -1;
	...
	if (inst->sql_inst->sql_fetch_row(sqlsocket, inst->sql_inst) == -1) {
		//radlog(L_ERR, "rlm_pms_2 : %s : Could not fetch row from select query.", "SC_HSPGW_AUTH_ORA_ERROR");
		(inst->sql_inst->module->sql_finish_select_query)(sqlsocket,
				inst->sql_inst->config);
		inst->sql_inst->sql_release_socket(inst->sql_inst, sqlsocket);
		return -1;
...
static int pms_checkedin(void *instance, REQUEST *request, pms_tracker_data **tracker) {
	...
	/*    if (!vp){
	 //radlog(L_ERR, "rlm_pms : %s : %s : Missing SCS-SPP-DecodedPassword attribute.", logfiledata->sessid, logfiledata->locationid);
	 //radlog(L_ERR, "rlm_pms_2 : %s : Missing SCS-SPP-DecodedPassword attribute.", "SC_HSPGW_ACCT_MALFORMED_REQ");
	 return -2;
	 }n */
	strcpy(etwas,"%");
...
static int pms_fias_accTrackerReloginInsert(void *instance, REQUEST *request, pms_tracker_data *trckrdata, int trackerId, char* mac, char* ip, int group, int webproxy) {
	...
	if (sqlsocket == NULL) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %s : rlm_pms_2 : Connection to data base failed.",
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, insertquery)) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %s : rlm_pms_2 : Insertion of account tracker relogin data failed.",
...
static int pms_fias_occupancyReloginInsert(void *instance, REQUEST *request, pms_tracker_data *trckrdata, int occupancyId, char *mac, char *ip, int group, int webproxy) {
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, insertquery)) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %s : rlm_pms_2 : Insertion of occupancy relogin data failed.",
				"SC_HSPGW_AUTH_ORA_ERROR", trckrdata->sessid,
				trckrdata->zoneid);
		(inst->sql_inst->module->sql_finish_query)(sqlsocket,
				inst->sql_inst->config);
...
static int pms_fias_accTrackerInsert(void *instance, REQUEST *request,
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, insertquery)) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %s : rlm_pms_2 : Insertion of account tracker data failed.",
				"SC_HSPGW_AUTH_ORA_ERROR", trckrdata->sessid,
				trckrdata->zoneid);
		(inst->sql_inst->module->sql_finish_query)(sqlsocket,
				inst->sql_inst->config);
...
static int pms_fias_accUpdateGid(void *instance, REQUEST *request,
	...
	if (sqlsocket == NULL) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %s : rlm_pms_2 accUpdateGid : Connection to data base failed.",
				"SC_HSPGW_AUTH_SPPDB_CONN_ERR", trckrdata->sessid,
				trckrdata->zoneid);
		return -1;
	}
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, updquery)) {
		//        radlog(L_ERR, "rlm_pms_2 : %s : Update failed: %s", "SC_HSPGW_ORA_ERROR", updquery);
		radlog(
				L_ERR,
				"%s : sid %s : zid %s : rlm_pms_2 accUpdateGid : Update of guest-ID in account tracker failed.",
...
static int pms_fias_accUpdBilled(void *instance, REQUEST *request, pms_data *pmsdata) {
	...
	sqlsocket = inst->sql_inst->sql_get_socket(inst->sql_inst);
	if (sqlsocket == NULL) {
		radlog(L_ERR, "rlm_pms_2 : %s : Connection to data base failed.",
				"SC_HSPGW_ORA_ERROR");
		return -1;
	}
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, querystr)) {
		radlog(L_ERR, "rlm_pms_2 : %s : Update failed: %s",
...
static int upd_active_gw(void *instance, REQUEST *request, pms_data *pmsdata) {
	...
	if (sqlsocket == NULL) {
		msgtxt
				= "SC_HSPGW_AUTH_SPPDB_CONN_ERR : rlm_pms_2 upd_active_gw : Connection to data base failed. Authentication aborted.";
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, updquery)) {
		msgtxt
				= "SC_HSPGW_AUTH_ORA_ERROR : rlm_pms_2 upd_active_gw : Update of active GW to data base failed. Authentication aborted.";
		radlog(L_ERR, msgtxt);
		(inst->sql_inst->module->sql_finish_query)(sqlsocket,
				inst->sql_inst->config);
		inst->sql_inst->sql_release_socket(inst->sql_inst, sqlsocket);
 ...
 *  @ void *inst: pms_2 module instance
 *  Return int
 *
 *  Author: hm/wdw
 *  Last changed: 2011-07-01
...
static int pms_fias_accTrackerPurge(void *instance, REQUEST *request) {
	...
	if (inst->sql_inst->sql_query(sqlsocket, inst->sql_inst, deletequery)) {
		radlog(L_ERR, "rlm_pms_2 : %s : Delete from table failed.",
				"SC_HSPGW_ORA_ERROR");
		(inst->sql_inst->module->sql_finish_query)(sqlsocket,
				inst->sql_inst->config);
		inst->sql_inst->sql_release_socket(inst->sql_inst, sqlsocket);
...
static int pms_fias_dbLogger(void *instance, REQUEST *request,
	...
	if (sqlsocket == NULL) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %d : rlm_pms_2 dbLogger : Connection to data base failed.",
	...
	if (ret != 0) {
		radlog(
				L_ERR,
				"%s : sid %s : zid %d : rlm_pms_2 dbLogger : Insert into log table failed.",
...
static void* pms_fias_thread_proc(void *arg) {
	...
	rlm_pms_2_module_t *inst = thread_arg->instance;
 ...
 *                function of the rlm_pms_2 module
 *
 ************************************************************************/
...
module_t rlm_pms_2 = { RLM_MODULE_INIT, "pms_2", RLM_TYPE_CHECK_CONFIG_SAFE, /* type */
